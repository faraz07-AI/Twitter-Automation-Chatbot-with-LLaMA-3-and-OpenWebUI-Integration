...
async def post_streaming_url(url: str, payload: Union[str, bytes], stream: bool = True):
    ...
    try:
        ...
        def post_message(full_content):
            tweet_text = "@airepublic_hire AI Republic said:"+full_content
            client.create_tweet(text=tweet_text)
            return

        async def custom_streaming_response_generator(response_content):
            nonlocal full_content
            async for chunk in response_content.iter_chunked(1024):
                content_chunk = chunk.decode('utf-8')
                try:
                    # Process each line separately if multiple JSON objects are present in a chunk
                    for line in content_chunk.splitlines():
                        ...
                        if "message" in chunk_json and "content" in chunk_json["message"]:
                            ...
                            full_content += chunk_json["message"]["content"]
                except json.JSONDecodeError:
                    # If chunk is not valid JSON, continue to next chunk
                    pass
                yield content_chunk

            # Log or process the full content here after streaming is done
            print("Captured full response content:", full_content)
            post_message(full_content)

        if stream:
            return StreamingResponse(
                custom_streaming_response_generator(r.content),
...
async def generate_chat_completion(
    ...
    log.info(f"url: {url}")
    log.debug(payload)
    print("content:"+json.dumps(payload))
...
async def generate_openai_chat_completion(
    ...
    response = await post_streaming_url(
        f"{url}/v1/chat/completions",
        json.dumps(payload),
        stream=payload.get("stream", False),
    )

    # Reading the content from StreamingResponse
    content = ""
    async for chunk in response.body_iterator:
        content += chunk.decode('utf-8')

    print("Captured response content:", content)
